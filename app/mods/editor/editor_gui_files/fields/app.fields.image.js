window.contApp.fieldDefinitions.image = _.defaults( new (function( px, contApp ){
	...
	this.bind = function( fieldData, mode ){
		...
		var res = resMgr.getResource( rtn.resKey );
		if( mode == 'finalize' ){
			rtn.path = resMgr.getResourcePublicPath( rtn.resKey );
		}
		if( mode == 'canvas' ){
			rtn.path = 'data:'+res.type+';base64,' + res.base64;

			if( !rtn || rtn == 'about:blank' ){
				// ↓ ダミーの Sample Image
	...
	this.mkEditor = function( mod, data ){
		...
		var res = resMgr.getResource( data.resKey );

		rtn.append($('<img>')
			.attr({
				"src":'data:'+res.type+';base64,' + res.base64
			})
			.css({'max-width':'100%'})
		);
		rtn.append($('<input>')
	...
	this.saveEditorContent = function( $dom, data ){
		...
		if( resMgr.getResource(data.resKey) === false ){
			data.resKey = resMgr.addResource();
		}
		...
		if( realpathSelected ){
			...
			resMgr.updateResource( data.resKey, tmpResInfo );
		}
		var res = resMgr.getResource( data.resKey );
		data.path = resMgr.getResourcePublicPath( data.resKey );
