window.contApp.fieldDefinitions.image = _.defaults( new (function( px, contApp ){
	...
	this.mkEditor = function( mod, data ){
		...
		rtn.append($('<img>')
			.attr({
				"src":'data:'+data.original.type+';base64,' + data.original.base64
			})
			.css({'max-width':'100%'})
		);
		rtn.append($('<input>')
	...
	this.saveEditorContent = function( $dom, data ){
		...
		if( typeof(data.original) !== typeof({}) ){
			data.original = {};
		}
		var realpathSelected = $dom.find('input').val();
		if( realpathSelected ){
			data.original.realpath = $dom.find('input').val();
			data.original.realpath = JSON.parse( JSON.stringify( data.original.realpath ) );
			data.original.ext = px.utils.getExtension( data.original.realpath );
			switch( data.original.ext.toLowerCase() ){
				case 'gif':                          data.original.type = 'image/gif';  break;
				case 'png':                          data.original.type = 'image/png';  break;
				case 'jpg': case 'jpeg': case 'jpe': data.original.type = 'image/jpeg'; break;
				case 'svg':                          data.original.type = 'image/svg+xml'; break;
				default:
					data.original.type = 'image/gif'; break;
			}

			var bin = px.fs.readFileSync( data.original.realpath, {} );
			data.original.base64 = px.utils.base64encode( bin );
			data.path = 'data:'+data.original.type+';base64,' + data.original.base64;
		}
